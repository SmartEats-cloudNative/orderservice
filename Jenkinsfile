   ...
   stage('Check code coverage') {
            steps {
                script {
                    ...
                    def componentKey = "com.example:orderservice"
      ...
      steps {
          ...
          sh 'docker build -t lavanyasimham/order-service:${VERSION} .'
          sh 'docker push lavanyasimham/order-service:${VERSION}'
      }
    } 
    ...
    stage('Update Image Tag in GitOps') {
      steps {
        ...
        script {
       sh '''
          sed -i "s/image:.*/image: lavanyasimham\\/order-service:${VERSION}/" aws/order-manifest.yml
        '''
